...
struct DashboardScreen: View {
            ...
            VStack(spacing: 0) {
                ScrollView {
                    VStack(spacing: 24) {
                        ...
                        // Summary Cards
                        HStack(spacing: 16) {
                            SummaryCard(title: "Today", value: String(format: "$%.2f", todayTotal()))
                            SummaryCard(title: "This Month", value: String(format: "$%.2f", monthTotal()))
                        }
                        .padding(.horizontal)
                ...
                HStack(spacing: 20) {
                    ...
                    NavigationLink(destination: ExpenseListScreen(context: viewContext)) {
                        DashboardButton(system: "list.bullet", text: "Expense List", color: .green.opacity(0.2))
                    }
                    
                    NavigationLink(destination: AddExpenseScreen(context: viewContext)) {
    ...
    // MARK: - Helpers for totals
    private func todayTotal() -> Double {
        let today = Calendar.current.startOfDay(for: Date())
        return expenses.filter { $0.date ?? Date() >= today }.reduce(0) { $0 + $1.amount }
    }
    ...
    struct SummaryCard: View {
        ...
        var body: some View {
            ...
            .padding()
            .frame(maxWidth: .infinity, alignment: .leading)
            .background(RoundedRectangle(cornerRadius: 16).fill(Color(.systemBackground)).shadow(color: .black.opacity(0.05), radius: 6, y: 4))
        }
    }
